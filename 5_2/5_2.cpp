...
//2.  Переопределите этот метод в дочернем классе так, чтобы возвращалась сумма урона от оружия и дополнительного урона(всё - таки это магическое оружие).
//3.  Проверьте, что объект магического оружия возвращает урон, как сумму базового урона(который идёт из базового класса) и дополнительного урона(который идёт из класса магического оружия).
//4.  Переопределите оператор > и <, которые будут сравнивать оружие по урону.То есть то оружие больше, которое имеет больший урон.Само значение урона получайте только через соответствующую функцию(поскольку, например, урон у магического оружия - это не просто поле, а сумма двух полей).
//5.  Проверьте работу операторов > и < , сравнивая разные экземпляры оружия и магического оружия.
//
